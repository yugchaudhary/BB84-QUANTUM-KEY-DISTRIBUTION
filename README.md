...
A visual, dark-themed dashboard with premium glassmorphism that allows you to:
-   **Interactive Photon Animation:** Watch glowing "photons" travel across the fiber optic channel from Alice to Bob.
-   **Quantum AI Assistant:** A brainy chatbot (powered by Google Gemini) that explains simulation results and physics concepts.
-   **One-Time Pad (OTP) Encryption:** Real-time text encryption using the quantum key generated by the simulation.
-   **Eavesdropper Simulation:** Toggle Eve to observe the 25% QBER (Quantum Bit Error Rate) signature of interception.
